...
- **Primary Blue (`#4A90D9`)**: Used for primary actions, active navigation, links, and focus rings (`ring-2 ring-primary`). For clickable text and links, use `#2E6BB5` for WCAG AA compliance (>4.5:1 ratio).
- **Secondary Amber (`#F59E0B`)**: Attention and warnings only (spam verdicts, pending DNS, advisories). For text/icons, use `#B45309`. Do not use Amber as a primary action button or generic decoration.
- **Red (`#EF4444`)**: Reserved for destructive actions, phishing verdict states, and critical alerts.
...
- View Transitions API (`startViewTransition`) visually connects screen states and slide-out inspection drawers (`same-document-transitions`).
