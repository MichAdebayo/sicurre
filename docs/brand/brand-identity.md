...
- **Primary Blue (`#4A90D9`)**: Identifies brand identity, primary actions, active navigation, and focus rings (`focus-visible:ring-2 focus-visible:ring-primary`, so the ring appears on keyboard focus and not on a mouse click). For clickable text and inline links, use the darker variant **`#2E6BB5`** to guarantee WCAG AA contrast (>4.5:1 ratio). Primary blue is an action color, not a verdict color.
- **Secondary Amber (`#F59E0B`)**: Identifies attention required, spam verdicts, pending DNS verification, trial/quota advisories, and warning highlight banners. Spam text uses **`#7A4700`** in light mode and **`#FBBF24`** in dark mode to remain distinct from phishing; general warning text and icons use **`#B45309`**.
