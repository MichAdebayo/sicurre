...
### Clean Code Rules
- **Modular structure:** Each script/module does ONE thing. Source-specific entrypoints live under `scripts/data_platform/<source>/<phase>/`, while reusable business logic lives under `src/data_platform/`. Scripts are orchestrators or no-write probes only; source-facing fetch/parse/persist logic belongs in `src/data_platform/extractors/` or `src/data_platform/services/`. No god-files.
