...
def extract_text_from_html(html: str) -> str:
    return CommonCrawlContentService.extract_text_from_html(html)
    ...
    async with semaphore:
        for attempt in range(4):
            try:
                ...
                if attempt == 3:
                    logger.debug(
                        f"Failed query {query.pattern} on {crawl_id} after retries: {exc}"
                    )
                await asyncio.sleep(2**attempt)
        return []
...
async def run_pipeline():
    ...
    limits = httpx.Limits(
        max_keepalive_connections=ASYNC_CONCURRENCY,
        max_connections=ASYNC_CONCURRENCY + 10,
    )
    ...
    async with httpx.AsyncClient(
        ...
        for crawl_id in CC_CRAWL_INDICES:
            if len(all_records) > MAX_WARC_DOWNLOADS * 1.5:
                logger.info(
                    f"Sufficient index hits collected ({len(all_records)}). Moving to Phase 2."
                )
                break

            tasks = [
                fetch_index_page(client, q, crawl_id, index_semaphore)
    ...
    df_index = (
        pd.DataFrame(all_records).drop_duplicates(subset=["url"]).reset_index(drop=True)
    )
    ...
    if "_url_priority_score" in df_index.columns:
    ...
    logger.info(
        f"Phase 2: Concurrently downloading WARC records (Max bounding: {len(df_index)})..."
    )
    extracted_pages = []
    ...
    async with httpx.AsyncClient(
        ...
        for idx in range(0, len(df_index), batch_size):
            ...
            tasks = [
                fetch_warc_record(client, row, semaphore, tracker) for row in batch_rows
            ]
            ...
            logger.info(
                f"  Progress: Downloaded={tracker.total_downloaded}, Extracted={tracker.extracted}"
            )
    ...
    df_usable = df_all[
        (df_all["language"] == "fr") & (df_all["text_length"] >= MIN_TEXT_LENGTH)
    ].reset_index(drop=True)
