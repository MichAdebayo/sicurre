...
DEFAULT_OUTPUT_JSON = (
    ROOT_DIR / "tasks/reviews/common-crawl-live-three-class-evaluation.json"
)
DEFAULT_OUTPUT_MD = (
    ROOT_DIR / "tasks/reviews/common-crawl-live-three-class-evaluation.md"
)
DEFAULT_DRAFTS_JSON = (
    ROOT_DIR / "tasks/reviews/common-crawl-live-three-class-drafts.json"
)
DEFAULT_EXPORT_JSON = (
    ROOT_DIR / "tasks/reviews/common-crawl-live-three-class-export.json"
)
DEFAULT_EXPORT_CSV = ROOT_DIR / "tasks/reviews/common-crawl-live-three-class-export.csv"
DEFAULT_MATRIX = ROOT_DIR / "tasks/reviews/common-crawl-merged-routing-matrix.json"
...
def _build_sample(
    *,
    ...
    return {
        ...
        "source_label": raw_payload.get("label") or "unknown",
        "source_category": raw_payload.get("category") or "unknown",
...
def _build_adaptation_queue(
    *,
        ...
        rules.append(
            {
                ...
                "rationale": metadata.get("rationale", "live source evaluation"),
    ...
    return {
        ...
        "sources": [{"source_name": SOURCE_NAME, "rules": rules}],
    }


def _render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# Common Crawl Live Three-Class Evaluation",
        "",
...
async def _load_raw_payloads() -> list[tuple[str, dict[str, Any]]]:
    ...
    async with session_factory() as session:
        ...
        result = await session.execute(
            ...
            .order_by(DataRawRecord.created_at.asc())
        )
        records = result.scalars().all()
        payloads = [
            (str(record.id), json.loads(record.raw_content)) for record in records
        ]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate the live Common Crawl DB source across legitimate, spam, and phishing lanes in no-write mode."
    )
    ...
    for rule_key in ADAPTABLE_SUBTYPES:
        if counter := Counter(
            ...
            if draft.get("source_name") == SOURCE_NAME
